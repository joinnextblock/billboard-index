...
function App() {
  ...
  function init(pool: SimplePool) {
    const filters = [
      { kinds: [31990, 0, 10002], "#t": ["billboard"] }, // BILLBOARD specific events
    ];
    const handlers = {
      onevent: (event: any) => {
  ...
  const billboard_events = from(queryStore.timeline({ kinds: [31990], "#t": ["billboard"] }));
        ...
        <div class="header">The Billboard Index</div>
          <For each={billboard_events()}>
            {(event) => {
              ...
              const kinds = tags.filter(([key]) => key === 'k').map(([_, value]) => parseInt(value)).sort((a, b) => a - b);
              return (<Billboard name={name} description={description} image={image} url={url} nips={nips} kinds={kinds} />)
            }}
          </For>
